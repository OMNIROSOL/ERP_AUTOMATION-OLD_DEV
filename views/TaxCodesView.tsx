      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Tax Codes</h1>
              <p className="text-sm text-slate-500 font-medium tracking-tight">Manage VAT and other applicable tax rates</p>
            </div>
          </div>
        </div>
        ...
        <button 
          onClick={() => {
            setEditingCode({ name: '', rate: 0, description: '', inactive: false });
              ...
              {filteredCodes.map((code) => (
                <tr key={code.id} className="hover:bg-slate-50/80 transition-colors group">
                  <td className="px-6 py-4">
                    <span className="font-bold text-slate-900">{code.name}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-black bg-blue-50 text-blue-700">
                      {code.rate}%
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-slate-500 font-medium">{code.description || '-'}</span>
                  </td>
                  <td className="px-6 py-4">
                    <button 
                    ...
                    >
                      {code.inactive ? <XCircle size={12} /> : <CheckCircle2 size={12} />}
                      {code.inactive ? 'Inactive' : 'Active'}
                    </button>
                  </td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm font-bold text-slate-500">No tax codes found</p>
                    </div>
                  </td>
                </tr>
              )}
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-200">
            <div className="bg-slate-50 p-6 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-900 tracking-tight">
                {editingCode?.id ? 'Edit Tax Code' : 'New Tax Code'}
              </h2>
              <button 
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tax Name</label>
                <input 
                  autoFocus
                  required
                  type="text" 
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input 
                    ...
                    type="number" 
                    step="0.01"
                    value={editingCode?.rate || ''}
                    onChange={(e) => setEditingCode({ ...editingCode, rate: Number(e.target.value) })}
                    placeholder="0.00"
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary/40 focus:bg-white transition-all outline-none"
                  />
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  ...
                  placeholder="Optional notes about this tax code..."
                  rows={3}
