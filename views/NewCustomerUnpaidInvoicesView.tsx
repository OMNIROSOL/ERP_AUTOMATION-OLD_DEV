...
const NewCustomerUnpaidInvoicesView: React.FC = () => {
  ...
  const handleSave = () => {
    const reportData = {
      id: isEdit ? id : `unpaid-report-${Date.now()}`,
      date: formData.date,
    ...
    localStorage.setItem('unpaid_invoice_reports', JSON.stringify(reports));
    navigate('/reports/unpaid-invoices');
  };
      ...
      <div className="flex justify-between items-center">
          <div>
              <div className="flex items-center space-x-2 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2 font-sans">
                  <span className="cursor-pointer hover:underline" onClick={() => navigate('/reports/unpaid-invoices')}>Unpaid Invoices</span>
                  <ChevronRight size={10} className="opacity-50" />
                  <span className="text-slate-400">{isEdit ? 'Modify Existing' : 'Generate New'}</span>
              </div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight font-sans">{isEdit ? 'Edit Report' : 'Report Configuration'}</h1>
          </div>
          <button
        ...
        <div className="p-12 space-y-12">
            <div className="space-y-8 font-sans">
                <div className="flex items-center space-x-4 border-b border-slate-50 pb-4">
                    ...
                    <div>
                        <h2 className="text-lg font-black text-slate-800 tracking-tight">Scope Parameters</h2>
                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Define which invoices to include</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative group">
                            ...
                            >
                                ...
                                <option value="Balance">Overdue Balance</option>
                            </select>
                ...
                <div className="grid grid-cols-1 gap-8">
                    <InputField 
                        ...
                        Icon={Type} 
                        placeholder="e.g. List of outstanding invoices for follow-up" 
                    />
                </div>
            </div>
...
export default NewCustomerUnpaidInvoicesView;
