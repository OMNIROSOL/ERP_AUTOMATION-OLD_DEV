...
import { mockSalesOrders, getCustomers } from '../mockData';
...
const ViewSalesOrderView = () => {
    ...
    const orderIndex = mockSalesOrders.findIndex(o => o.id === id);
    const order = mockSalesOrders[orderIndex];

    const customerData = useMemo(() => {
        if (!order) return null;
        return allCustomers.find(c => c.name === order.customer);
    }, [order, allCustomers]);

    const customerEmail = customerData?.email || (order ? `${order.customer.toLowerCase().replace(/\s+/g, '.')}@example.com` : '');

    const totals = useMemo(() => {
        if (!order) return { subtotal: 0, tax: 0, total: 0 };
        if (!order.items || order.items.length === 0) return { subtotal: order.amount, tax: 0, total: order.amount };
        let subtotal = 0;
        let tax = 0;
        ...
        return { subtotal, tax, total: order.amount };
    }, [order]);
        ...
        <div className="min-h-screen bg-[#f3f4f6]/50 flex flex-col font-sans">
            ...
            <div className="bg-[#f8fafc] border-b border-gray-300 px-6 py-3 flex items-center justify-between sticky top-0 z-50 no-print">
                    ...
                    <div className="flex space-x-2">
                        ...
                        <button 
                            onClick={async () => {
                                ...
                                pdf.save(`${order.reference || 'Order'}.pdf`);
                            }} 
                            className="bg-white border border-gray-300 px-4 py-1.5 text-[12px] font-bold text-gray-700 rounded shadow-sm hover:bg-gray-50 flex items-center gap-2"
                        >
                ...
                <div className="flex items-center space-x-2">
                    <div className="flex bg-white border border-gray-300 rounded shadow-sm">
                        <button
                            onClick={() => navigate(`/sales-orders/view/${mockSalesOrders[0].id}`)}
                        ...
                        <button
                            onClick={() => navigate(`/sales-orders/view/${mockSalesOrders[Math.max(0, orderIndex - 1)].id}`)}
                            disabled={orderIndex <= 0}
                    ...
                    <span className="text-[11px] font-bold text-gray-400 mx-2 uppercase tracking-widest">{orderIndex + 1} / {mockSalesOrders.length}</span>
                    <div className="flex bg-white border border-gray-300 rounded shadow-sm">
                        <button
                            onClick={() => navigate(`/sales-orders/view/${mockSalesOrders[Math.min(mockSalesOrders.length - 1, orderIndex + 1)].id}`)}
                            disabled={orderIndex === mockSalesOrders.length - 1}
                            className="px-3 py-1.5 text-gray-600 hover:bg-gray-50 border-r border-gray-300 disabled:opacity-30 flex items-center"
                        >
                            <ChevronRight size={14} />
                        </button>
                        <button
                            onClick={() => navigate(`/sales-orders/view/${mockSalesOrders[mockSalesOrders.length - 1].id}`)}
                            disabled={orderIndex === mockSalesOrders.length - 1}
                            className="px-3 py-1.5 text-gray-600 hover:bg-gray-50 disabled:opacity-30 flex items-center"
                        >
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-12 items-start">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-slate-900 uppercase tracking-tight mb-2">{order.customer}</p>
                                    <div className="text-gray-500 space-y-1">
                                        <p className="whitespace-pre-wrap">{customerData?.billingAddress || order.billingAddress || '-'}</p>
                        ...
                        <div className="w-80 space-y-3">
                            ...
                            <div className="flex justify-between items-center bg-slate-50 p-4 border-t-2 border-slate-900 mt-2 print-bg-slate-50">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-2xl font-bold text-slate-900 tracking-tighter">{order.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="bg-[#f3f4f6] px-8 py-4 border-t border-gray-200 flex justify-end no-print">
                <div className="flex space-x-2">
                    ...
                    <button onClick={() => navigate(`/sales-orders/edit/${order.id}`)} className="bg-blue-600 text-white px-6 py-2 text-[11px] font-bold rounded-md shadow-md hover:bg-blue-700 transition uppercase tracking-widest flex items-center gap-2">
...
export default ViewSalesOrderView;
