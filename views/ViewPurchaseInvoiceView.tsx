...
const ViewPurchaseInvoiceView = () => {
    ...
    const supplierData = useMemo(() => {
        ...
        return allSuppliers.find(s => s.name === invoice.supplier);
    }, [invoice, allSuppliers]);

    const supplierEmail = supplierData?.email || (invoice ? `${invoice.supplier.toLowerCase().replace(/\s+/g, '.')}@example.com` : '');
        ...
        <div className="min-h-screen bg-[#f3f4f6]/50 flex flex-col font-sans">
            ...
            <div className="bg-[#f8fafc] border-b border-gray-300 px-6 py-3 flex items-center justify-between sticky top-0 z-50 no-print">
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        onClick={() => navigate(`/purchase-invoices/edit/${invoice.id}`)}
                        className="bg-white border border-gray-300 px-4 py-1.5 text-[12px] font-bold text-gray-700 rounded shadow-sm hover:bg-gray-50 flex items-center gap-2"
                    >
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-12 items-start">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-slate-900 uppercase tracking-tight mb-2">{invoice.supplier}</p>
                                    <div className="text-gray-500 space-y-1">
                                        <p className="whitespace-pre-wrap">{supplierData?.billingAddress || invoice.billingAddress || '-'}</p>
            ...
            <div className="bg-[#f3f4f6] px-8 py-4 border-t border-gray-200 flex justify-end no-print">
                <div className="flex space-x-2">
                    ...
                    <button onClick={() => navigate(`/purchase-invoices/edit/${invoice.id}`)} className="bg-blue-600 text-white px-6 py-2 text-[11px] font-bold rounded-md shadow-md hover:bg-blue-700 transition uppercase tracking-widest flex items-center gap-2">
