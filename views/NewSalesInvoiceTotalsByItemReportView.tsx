...
const NewSalesInvoiceTotalsByItemReportView: React.FC = () => {
  ...
  const handleSave = () => {
    const reportData = {
      id: isEdit ? id : `sii-report-${Date.now()}`,
      fromDate: formData.fromDate,
      toDate: formData.toDate,
      division: formData.division,
      description: formData.description,
      sortBy: formData.sortBy,
      totalAmount: Math.floor(Math.random() * 150000) + 30000,
    ...
    localStorage.setItem('sales_invoice_item_totals_reports', JSON.stringify(reports));
    navigate('/reports/sales-invoice-totals-by-item');
  };

  return (
    <div className="p-10 max-w-5xl mx-auto space-y-10 animate-in fade-in duration-700 font-sans text-left">
      <div className="flex justify-between items-center">
          <div>
              <div className="flex items-center space-x-2 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">
                  <span className="cursor-pointer hover:underline" onClick={() => navigate('/reports/sales-invoice-totals-by-item')}>Sales Invoice Totals (Item)</span>
                  <ChevronRight size={10} className="opacity-50" />
                  <span className="text-slate-400">{isEdit ? 'Modify Existing' : 'Generate New'}</span>
              </div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{isEdit ? 'Edit Item Report' : 'Item Report Configuration'}</h1>
          </div>
          <button
            ...
            <div className="space-y-8">
                <div className="flex items-center space-x-4 border-b border-slate-50 pb-4">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Define the product analysis range</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-8 pt-4">
                    <InputField 
                        ...
                        Icon={Type} 
                        placeholder="e.g. Q1 Product Performance Audit" 
                    />
                </div>
            </div>
...
export default NewSalesInvoiceTotalsByItemReportView;
